...
extern "C" {
    ...
    Java_com_module_net_socket_CSocket_close(JNIEnv *env, jobject thiz) {
        ...
        if( index>=0 ){
            pthread_mutex_lock(&java_socket[index].lock);
            java_socket[index].done = 1;
            pthread_mutex_unlock(&java_socket[index].lock);

//         waiting for the network thread to flip the done flag
            struct timespec sleepTime;
            memset(&sleepTime, 0, sizeof(sleepTime));
            sleepTime.tv_nsec = 100000000;
            while (java_socket[index].done) {
                nanosleep(&sleepTime, NULL);
            }

            env->DeleteGlobalRef(java_socket[index].j_socket);
            java_socket[index].j_socket = NULL;

            if(java_socket[index].c_host){
                free(java_socket[index].c_host);
            }
            java_socket[index].c_host = NULL;

            java_socket[index].c_port = 0;

            env->DeleteGlobalRef(java_socket[index].j_socket_handler);
            java_socket[index].j_socket_handler = NULL;

            java_socket[index].j_socket_handler_onSocketConnectResult = NULL;
            java_socket[index].j_socket_handler_onSocketResponse = NULL;

            java_socket[index].sfd = -1;

            pthread_mutex_destroy(&java_socket[index].lock);
